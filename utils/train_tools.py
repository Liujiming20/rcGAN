...
def pre_train(net_para, pretrain_model_path, drop_para, lr):
    ...
    train_dataset = DatasetFromCSV("./source_data/input_para/input_label.csv", "./source_data/input_para/input_value.csv")
    ...
    #
    #     with open("./result/R2_net_para/r2.csv", "a+", newline='') as csvfile:
    ...
    fig_filepath = "./result/pre-train_val/" + str(net_para[0]) + "_" + str(net_para[1]) + "_" + str(net_para[2]) + "_" + str(net_para[3]) + "_" + str(net_para[4]) + "_" + str(net_para[5]) + "_" + str(int(drop_para * 100)) + "_" + str(int(lr*100000)) +".png"
...
def test_pre(net_para, reg_pth_path, drop_para):
    ...
    with torch.no_grad():
        for i, (y_test, x_input) in enumerate(test_dataloader):
            ...
            DIR_mse = MSE[1]*55
            DIR_y2 = np.inner(y_test_ini[:, 1], y_test_ini[:, 1])
            DIR_nmse = DIR_mse/DIR_y2

            acce_mse = MSE[2]*55
            acce_y2 = np.inner(y_test_ini[:, 2], y_test_ini[:, 2])
            acce_nmse = acce_mse/acce_y2

            nmse = [fre_nmse, DIR_nmse, acce_nmse]

            with open("./result/R2_net_para/NMSE.csv", "a+", newline='') as csvfile:
                writer = csv.writer(csvfile)
    ...
    # DIR_test = y_test[:, 1]
    # acce_test = y_test[:, 2]
    # fre_predict = y_predict[:, 0]
    # DIR_predict = y_predict[:, 1]
    # acce_predict = y_predict[:, 2]
    #
    ...
    # axs[1].ticklabel_format(style='sci', scilimits=(-1, 2), axis="y", useMathText=True)
    # axs[2].ticklabel_format(style='sci', scilimits=(-1,2),axis="y",useMathText=True)
    # axs[0].plot(x, fre_test, label="fre_test", linestyle="-", color="pink")
    # axs[0].plot(x, fre_predict, label="fre_predict", linestyle="--", color="g")
    # axs[1].plot(x, DIR_test, label="DIR_test", linestyle="-", color="pink")
    # axs[1].plot(x, DIR_predict, label="DIR_predict", linestyle="--", color="g")
    # axs[2].plot(x, acce_test, label="acce_test", linestyle="-", color="pink")
    # axs[2].plot(x, acce_predict, label="acce_predict", linestyle="--", color="g")
    # axs[0].legend(loc=0)
    # axs[1].legend(loc=0)
    # axs[2].legend(loc=0)
    # fig_filepath = "./result/test_dataset_val/" + str(net_para[0]) + "_" + str(net_para[1]) + "_" + str(net_para[2]) + "_" + str(net_para[3]) + "_" + str(net_para[4]) + "_" + str(net_para[5]) + "_" + str(int(drop_para * 100))  + "_pre-reg.png"
    # plt.savefig(fig_filepath)
    # # plt.show()
    # plt.close()
...
def load_freeze_regression(net_para_reg, reg_pre_path, device):
    ...
    # regression = Regression03(3, 17, 0, True, 0.15, net_para_reg[0], net_para_reg[1], net_para_reg[2], net_para_reg[3], net_para_reg[4], net_para_reg[5])  # 无注意力机制层
...
def train(net_para, reg_net_para, lambda_para, gen_pth_path, reg_pre_path, k_count, drop_index, lr):
    ...
    fig_filepath = "./result/loss_func_val/" + str(net_para[0]) + "_" + str(net_para[1]) + "_" + str(net_para[2]) + "_" + str(net_para[3]) + "_" + str(net_para[4]) + "_" + str(net_para[5]) + "_" + str(int(drop_para * 100)) +"_" + str(int(lr*100000)) + "_" + str(int(lambda_para)) + ".png"
    plt.savefig(fig_filepath)
    # plt.show()
    plt.close()
    ...
    # fig.tight_layout(pad=0.4, w_pad=0.5, h_pad=1.0)
    # axs[0].plot(epochs, disc_loss_epochs_val, label="disc_loss", linestyle="-", color="orangered")
    # axs[0].plot(epochs, gen_BCE_loss_epochs_val, label="gen_BCE_loss", linestyle="-", color="steelblue")
    # axs[1].plot(epochs, gen_L1_loss_epochs_val, label="gen_SL1_loss", linestyle="-", color="darkorange")
    # axs[0].legend(loc=0)
    # axs[1].legend(loc=0)
    # fig_filepath = "./result/loss_func_val/" + str(net_para[0]) + "_" + str(net_para[1]) + "_" + str(
    #     net_para[2]) + "_" + str(net_para[3]) + "_" + str(net_para[4]) + "_" + str(net_para[5]) + "_" + str(
    #     int(drop_para * 100)) + ".png"
    # plt.savefig(fig_filepath)
    # # plt.show()
    # plt.close()
...
def test(net_para, net_para_reg, gen_pth_path, reg_pth_path, drop_para, lr, lambda_para, z_noise_test, seed):
    torch.manual_seed(111)
    ...
    regression = Regression03(3, 17, 1, True, 0.15, net_para_reg[0], net_para_reg[1], net_para_reg[2], net_para_reg[3], net_para_reg[4], net_para_reg[5])
    # regression = Regression03(3, 17, 0, True, 0.15, net_para_reg[0], net_para_reg[1], net_para_reg[2], net_para_reg[3], net_para_reg[4], net_para_reg[5])  # 无自注意力机制层
    ...
    with torch.no_grad():
        for i, (y_test, x_input) in enumerate(test_dataloader):
            ...
            # with open("./result/R2_net_para/y_test.csv", "a+", newline='') as csvfile:
            #     writer = csv.writer(csvfile)
            #     writer.writerows(y_test_ini)
            #
            # with open("./result/R2_net_para/y_predict.csv", "a+", newline='') as csvfile:
            #     writer = csv.writer(csvfile)
            ...
            with open("./result/R2_net_para/NRMSE.csv", "a+", newline='') as csvfile:
                writer = csv.writer(csvfile)
            ...
            with open("./result/R2_net_para/NMAE.csv", "a+", newline='') as csvfile:
                writer = csv.writer(csvfile)
    ...
    # axs[1].legend(loc=0)
    # axs[2].legend(loc=0)
    # fig_filepath = "./result/test_dataset_val/" + str(net_para[0]) + "_" + str(net_para[1]) + "_" + str(net_para[2]) + "_" + str(net_para[3]) + "_" + str(net_para[4]) + "_" + str(net_para[5]) + "_" + str(int(drop_para * 100))  +"_" + str(int(lr*100000)) + "_" + str(int(lambda_para)) + ".png"
    # # fig_filepath = "./result/test_dataset_val/" + str(net_para[0]) + "_" + str(net_para[1]) + "_" + str(net_para[2]) + "_" + str(net_para[3]) + "_" + str(net_para[4]) + "_" + str(net_para[5]) + "_" + str(int(drop_para * 100))  +"_" + str(int(lr*100000)) + "_" + str(int(lambda_para)) + "_" + str(seed) + ".png"  # 测试噪声时使用
    # plt.savefig(fig_filepath)
    # # plt.show()
    # plt.close()